...
//	/** select table name */
	 ...
	 * @param style
	 */
	public TadpoleTableComposite(IWorkbenchPartSite partSite, TabFolder parent, UserDBDAO userDB) {
		super(partSite, parent, userDB);
	 ...
	 */
	public void refreshTable(final UserDBDAO selectUserDb, final String source) {
		...
		try {
			...
			if ("DB".equals(source)) ExceptionDetailsErrorDialog.openError(null, "Error", Messages.ExplorerViewer_4, errStatus); //$NON-NLS-1$ //$NON-NLS-2$
			else  ExceptionDetailsErrorDialog.openError(null, "Error", Messages.ExplorerViewer_86, errStatus); //$NON-NLS-1$
		}
	}
	 ...
	 * Table 정의
	 */
	private void createTable(final TabFolder tabFolderObject) {
