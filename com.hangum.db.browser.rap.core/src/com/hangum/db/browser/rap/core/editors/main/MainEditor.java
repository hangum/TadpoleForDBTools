...
import com.hangum.db.util.tables.SQLLibraryLabelProvider;
import com.hangum.db.util.tables.SQLLibrarySorter;
...
public class MainEditor extends EditorPart {
	...
	private Text libraryFilter;

	private SQLResultFilter sqlFilter = new SQLResultFilter();
	private SQLResultFilter libFilter = new SQLResultFilter();
	...
	/** tab index name */
	private enum RESULT_TAB_NAME {
		RESULT_SET, SQL_RECALL, SQL_LIBRARY, TADPOLE_MESSAGE 
	};
	...
	/** SQL Library */
	private TableViewer tableViewerSQLLibrary;
	private Table tableSQLLibrary;
	private List<SQLLibraryDAO> listSQLLibrary = new ArrayList<SQLLibraryDAO>();
	...
	public void createPartControl(Composite parent) {
		...
		tabFolderResult.setData(RESULT_TAB_NAME.SQL_LIBRARY.toString(), 3);
		...
		gl_compositeSQLHistory.marginHeight = 2;
		...
		// ////////////////SQL Library Start /////////////////////////
		TabItem tbtmSQLLibrary = new TabItem(tabFolderResult, SWT.NONE);
		// tbtmSQLLibrary.setText(Messages.MainEditor_10);
		tbtmSQLLibrary.setText("SQL Library");

		Composite compositeSQLLibrary = new Composite(tabFolderResult, SWT.NONE);
		tbtmSQLLibrary.setControl(compositeSQLLibrary);
		GridLayout gl_compositeSQLLibrary = new GridLayout(2, false);
		gl_compositeSQLLibrary.marginHeight = 2;
		compositeSQLLibrary.setLayout(gl_compositeSQLLibrary);

		Label lblFilter2 = new Label(compositeSQLLibrary, SWT.NONE);
		lblFilter2.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		lblFilter2.setText(Messages.MainEditor_lblFilter_text);

		libraryFilter = new Text(compositeSQLLibrary, SWT.BORDER);
		libraryFilter.addKeyListener(new KeyAdapter() {
			@Override
			public void keyPressed(KeyEvent e) {
				if (e.keyCode == SWT.Selection)
					setLibFilter();
		...
		libraryFilter.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));

		tableViewerSQLLibrary = new TableViewer(compositeSQLLibrary, SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL | SWT.FULL_SELECTION);
		tableViewerSQLLibrary.addDoubleClickListener(new IDoubleClickListener() {
			public void doubleClick(DoubleClickEvent event) {
				...
				if (!is.isEmpty()) {
					try {
						Object selElement = is.getFirstElement();
						if (selElement instanceof SQLLibraryDAO) {
							SQLLibraryDAO daoLibrary = (SQLLibraryDAO) selElement;	

							StringBuffer sql = new StringBuffer("/*");
							sql.append(daoLibrary.getTitle());
							sql.append("\r\n");
							sql.append(daoLibrary.getDescription());
							sql.append("*/\r\n");
							sql.append(daoLibrary.getSQLText());
							sql.append( Define.SQL_DILIMITER);
							
							setAppendQueryText(sql.toString()); //$NON-NLS-1$
							browserQueryEditor.evaluate(EditorBrowserFunctionService.JAVA_SCRIPT_APPEND_QUERY_TEXT);	
						}
					} catch (Exception ee) {
						logger.error("history selection", ee); //$NON-NLS-1$
		...
		tableSQLLibrary = tableViewerSQLLibrary.getTable();
		tableSQLLibrary.setLinesVisible(true);
		tableSQLLibrary.setHeaderVisible(true);
		tableSQLLibrary.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 2, 1));
		tableSQLLibrary.setBounds(0, 0, 85, 85);
		tableSQLLibrary.setSortDirection(SWT.DOWN);

		libFilter.setTable(tableSQLLibrary);

		SQLLibrarySorter sorterLibrary = new SQLLibrarySorter();
		TableViewerColumn tableLibraryViewerColumn0 = new TableViewerColumn(tableViewerSQLLibrary, SWT.NONE);
		TableColumn tblclmnLibrarySeqNo = tableLibraryViewerColumn0.getColumn();
		tblclmnLibrarySeqNo.setWidth(35);
		// tblclmnLibraryDate.setText(Messages.MainEditor_14);
		tblclmnLibrarySeqNo.setText("No");
		tblclmnLibrarySeqNo.addSelectionListener(getSelectionAdapter(tableViewerSQLLibrary, sorterLibrary, tblclmnLibrarySeqNo, 0));

		TableViewerColumn tableLibraryViewerColumn1 = new TableViewerColumn(tableViewerSQLLibrary, SWT.NONE);
		TableColumn tblclmnLibraryTitle = tableLibraryViewerColumn1.getColumn();
		tblclmnLibraryTitle.setWidth(140);
		// tblclmnLibraryDate.setText(Messages.MainEditor_14);
		tblclmnLibraryTitle.setText("Title");
		tblclmnLibraryTitle.addSelectionListener(getSelectionAdapter(tableViewerSQLLibrary, sorterLibrary, tblclmnLibraryTitle, 1));

		TableViewerColumn tableLibraryViewerColumn2 = new TableViewerColumn(tableViewerSQLLibrary, SWT.NONE);
		TableColumn tblclmnLibrarySql = tableLibraryViewerColumn2.getColumn();
		tblclmnLibrarySql.setWidth(500);
		tblclmnLibrarySql.setText(Messages.MainEditor_15);
		tblclmnLibrarySql.addSelectionListener(getSelectionAdapter(tableViewerSQLLibrary, sorterLibrary, tblclmnLibrarySql, 2));

		TableViewerColumn tableLibraryViewerColumn3 = new TableViewerColumn(tableViewerSQLLibrary, SWT.NONE);
		TableColumn tblclmnLibraryDesc = tableLibraryViewerColumn3.getColumn();
		tblclmnLibraryDesc.setWidth(500);
		// tblclmnLibrarySql.setText(Messages.MainEditor_15);
		tblclmnLibraryDesc.setText("Description");
		tblclmnLibraryDesc.addSelectionListener(getSelectionAdapter(tableViewerSQLLibrary, sorterLibrary, tblclmnLibraryDesc, 3));

		tableLibraryViewerColumn1.setEditingSupport(new SqlLibraryEditorSupport(tableViewerSQLLibrary, 1, this.userDB));
		tableLibraryViewerColumn2.setEditingSupport(new SqlLibraryEditorSupport(tableViewerSQLLibrary, 2, this.userDB));
		tableLibraryViewerColumn3.setEditingSupport(new SqlLibraryEditorSupport(tableViewerSQLLibrary, 3, this.userDB));

		sqlLibrary();

		tableViewerSQLLibrary.setLabelProvider(new SQLLibraryLabelProvider());
		tableViewerSQLLibrary.setContentProvider(new ArrayContentProvider());
		tableViewerSQLLibrary.setInput(listSQLLibrary);
		tableViewerSQLLibrary.setComparator(sorterLibrary);

		Composite compositeLibraryBtn = new Composite(compositeSQLLibrary, SWT.NONE);
		compositeLibraryBtn.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 2, 1));
		compositeLibraryBtn.setLayout(new GridLayout(2, false));

		final Button btnDeleteLibrary = new Button(compositeLibraryBtn, SWT.NONE);
		btnDeleteLibrary.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		btnDeleteLibrary.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				int newIndex = tableViewerSQLLibrary.getTable().getItemCount() + 1;
				listSQLLibrary.add(new SQLLibraryDAO(newIndex));
				tableViewerSQLLibrary.refresh();
			}
		});
		// btnDeleteLibrary.setText(Messages.MainEditor_12);
		btnDeleteLibrary.setText("Append");

		Button btnAppendLibrary = new Button(compositeLibraryBtn, SWT.NONE);
		btnAppendLibrary.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		btnAppendLibrary.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				SaveSqlLibrary();
			}
		});
		// btnAppendLibrary.setText(Messages.MainEditor_17);
		btnAppendLibrary.setText("Save");
		...
		gl_compositeMessage.marginHeight = 2;
		...
		tableLibraryViewerColumn1 = new TableViewerColumn(tableViewerMessage, SWT.NONE);
		tblclmnLibraryTitle = tableLibraryViewerColumn1.getColumn();
		tblclmnLibraryTitle.setWidth(140);
		tblclmnLibraryTitle.setText(Messages.MainEditor_14);
		tblclmnLibraryTitle.addSelectionListener(getSelectionAdapter(tableViewerMessage, sorterMessage, tblclmnLibraryTitle, 0));

		tableLibraryViewerColumn2 = new TableViewerColumn(tableViewerMessage, SWT.NONE);
		tblclmnLibrarySql = tableLibraryViewerColumn2.getColumn();
		tblclmnLibrarySql.setWidth(500);
		tblclmnLibrarySql.setText("Message"); //$NON-NLS-1$
		tblclmnLibrarySql.addSelectionListener(getSelectionAdapter(tableViewerMessage, sorterMessage, tblclmnLibrarySql, 1));
	...
	private void sqlHistory(String executeSQL) {
		...
		for (SQLHistoryDAO history : listSQLHistory) {
			if (history.getStrSQLText().equals(StringUtils.trimToEmpty(executeSQL)))
				isAlready = true;
		}
	...
	/**
	 * sql library 데이터를 만듭니다.
	 */
	private void sqlLibrary() {

		Map<String, Object> param = new HashMap<String, Object>();
		param.put("db_type", userDB.getTypes());
		param.put("db_seq", userDB.getSeq());
		param.put("user_seq", userDB.getUser_seq());

		SqlMapClient sqlClient;
		try {
			sqlClient = TadpoleSQLManager.getInstance(TadpoleSystemConnector.getUserDB());

			listSQLLibrary = sqlClient.queryForList("sqlLibraryList", param); //$NON-NLS-1$

			for (int i = 0; i < listSQLLibrary.size(); i++) {
				listSQLLibrary.get(i).setSeqNo(i + 1);
			}
	...
	/**
	 * sql library를 텍스트로 만듭니다.
	 * 
	 * @return
	 * @throws Exception
	 */
	private String getLibraryTabelSelectData() throws Exception {
		StringBuffer sbData = new StringBuffer();

		for (TableItem ti : tableSQLLibrary.getSelection()) {
			sbData.append(ti.getText(1));
		}
	...
	private void SaveSqlLibrary() {

		java.sql.Connection javaConn = null;
		PreparedStatement stmtInsert = null;
		PreparedStatement stmtDelete = null;		
		try {
			SqlMapClient client = TadpoleSQLManager.getInstance(TadpoleSystemConnector.getUserDB());

			javaConn = client.getDataSource().getConnection();

			StringBuffer queryInsert = new StringBuffer().append("insert into user_db_sqlibrary(title, db_type, sqltext, description, types, user_seq) values (?, ?, ?, ?, ?, ?)");
			StringBuffer queryDelete = new StringBuffer().append("delete from user_db_sqlibrary where seq = ? ");

			stmtInsert = javaConn.prepareStatement(queryInsert.toString());
			stmtDelete = javaConn.prepareStatement(queryDelete.toString());

			for (SQLLibraryDAO dao : listSQLLibrary) {
				if (!dao.isModify()) continue;
				
				int i = 1;
				stmtInsert.setObject(i++, dao.getTitle());
				stmtInsert.setObject(i++, userDB.getTypes());
				stmtInsert.setObject(i++, dao.getSQLText());
				stmtInsert.setObject(i++, dao.getDescription());
				stmtInsert.setObject(i++, dao.getTypes());
				stmtInsert.setObject(i++, userDB.getUser_seq());
				
				if (dao.getSeq() > 0 ){
					stmtDelete.setObject(1,  dao.getSeq());
					stmtDelete.addBatch();
				}
	 ...
	 * 쿼리를 수행합니다.
	 * 
	 * text중에 블럭으로 되어 있는 부분만 쿼리를 수행합니다. ;를 기준으로 여러개로 나누어 쿼리를 수행합니다.
	 * 
	 */
	public void execute() {
				...
				getSite().getShell().getDisplay().asyncExec(new Runnable() {
					public void run() {
						if (jobEvent.getResult().isOK()) {
							// query history
							for (String strQuery : listQueryHistory)
								sqlHistory(strQuery);
	...
	/**
	 * 1) 마지막 쿼리를 받아서 selct 문일 경우 쿼리 네비게이션을 확성화 해준다. 2) filter를 설정한다.
	 * 
	 * @param lastQuery
	 *            실행된 마지막 쿼리
	 */
	private void executeFinish() {
		...
		if (executeLastSQL.toUpperCase().startsWith("SHOW") || //$NON-NLS-1$
			...
			btnPrev.setEnabled(false);
			if (sourceDataList.size() < queryPageCount)
				btnNext.setEnabled(false);
			else
	...
	private void btnNext() {
		...
		if (logger.isDebugEnabled())
			logger.debug("btnNext ======> [start point]" + startCount + "\t [endCount]" + endCount);
	...
	private void btnPrev() {
		...
		int endCount = queryPageCount * (pageNumber - 1);
		if (logger.isDebugEnabled())
			logger.debug("btnPrev ======> [start point]" + startCount + "\t [endCount]" + endCount);
	...
	private void setLibFilter() {
		libFilter.setFilter(libraryFilter.getText());
		tableViewerSQLLibrary.addFilter(libFilter);
	}
		...
		try {
			...
			if (Define.QUERY_MODE.DEFAULT == queryMode) {
				...
				if (requestQuery.toUpperCase().startsWith("SELECT")) { //$NON-NLS-1$
					...
					if (logger.isDebugEnabled())
						logger.debug("[SELECT] " + requestQuery); //$NON-NLS-1$
				}
