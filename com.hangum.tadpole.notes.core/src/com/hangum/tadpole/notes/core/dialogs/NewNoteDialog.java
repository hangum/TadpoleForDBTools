...
public class NewNoteDialog extends Dialog {
	...
	private List<Integer> listAlluser = new ArrayList<Integer>();
	...
	protected void okPressed() {
		...
		try {
			// User data save
			TadpoleSystem_Notes.saveNote(comboTypes.getText(), listAlluser, SessionManager.getSeq(), userDao.getSeq(), strTitle, strContent);
		} catch(Exception e) {
	...
	private void initUI() {
		...
		try {
			...
			for (UserGroupAUserDAO userGroupAUserDAO : listUserGroup) {
				...
				listAlluser.add(userGroupAUserDAO.getUser_seq());
			}
