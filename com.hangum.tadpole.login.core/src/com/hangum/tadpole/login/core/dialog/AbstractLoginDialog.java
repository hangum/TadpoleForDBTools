...
import com.hangum.tadpole.engine.security.OTPValidation;
import com.hangum.tadpole.engine.utils.LDAPUtil;
...
public abstract class AbstractLoginDialog extends Dialog {
	...
	public void configureShell(Shell newShell) {
		...
		newShell.setText(SystemDefine.getProductName()); //$NON-NLS-1$
	...
	protected void ldapLogin(String strEmail, String strPass) throws TadpoleAuthorityException {
		LDAPUtil.getInstance().isValidation(strEmail, strPass);
	}
	...
	protected boolean isQuestOTP(UserDAO userDao, String strUserIP) {
		if(PublicTadpoleDefine.YES_NO.NO.name().equals(userDao.getUse_otp())) return true;
		...
		if(LoadConfigFile.isUseOPT()) {
			if(PublicTadpoleDefine.OTP_METHOD.APP_VERIFICATION.name().equalsIgnoreCase(LoadConfigFile.otpMethod())) {
				...
				if(false == validation.validation(userDao.getEmail())) {
