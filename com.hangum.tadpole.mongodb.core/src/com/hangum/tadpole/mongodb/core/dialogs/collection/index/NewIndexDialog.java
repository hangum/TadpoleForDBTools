...
public class NewIndexDialog extends Dialog {
	...
	private void initUI() {
		...
		try {
			...
			for (TableDAO tableDao : listCollFields)
				comboColName.add(tableDao.getName());
	...
	private void createTableMongoColumne() {
		String[] columnName = {
				"Field", "Type", "Key", "Select Index"}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
		int[] columnSize = {
				200,
				170,
				40,
		...
		try {
			...
			for (int i = 0; i < columnName.length; i++) {
				...
				if (i == 3)
					tableColumn.setEditingSupport(new FieldIndexEditorSupport(treeColumnViewer, userDB));
			} // end for
