...
public class MapReduceEditor extends EditorPart {
	...
	private String TEMPLATE_FINALIZE_SRC = "";// "function Finalize(key, reduced) {\r"
												// +
												// TadpoleWidgetUtils.TAB_CONETNT
												// + " return reduced; \r}";
	...
	public void createPartControl(Composite parent) {
		...
		sashFormMRF.setWeights(new int[] {
				4,
				4,
				2
		});
		...
		Job job = new Job("MapReduce search job") { //$NON-NLS-1$
			...
			public IStatus run(IProgressMonitor monitor) {
				...
				try {
					...
					// mrOutput = new MapReduceOutput(dbCol, basicObj,
					// cmdResult);
					// } else {
					cmdResult = dbCol.getDB().command(basicObj);
					// cmdResult.throwOnError();
