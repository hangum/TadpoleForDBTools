...
public class PublicTadpoleDefine {
	...
	/** Success, Fail */
	public static enum SUCCESS_FAIL {
		S,
		F
	};
	...
	public static enum TADPOLE_SUPPORT_BROWSER {
		FIREFOX,
		CHROME,
		SAFARI,
		IE10,
	...
	public static enum EDITOR_OPEN_TYPE {
		NONE,
		STRING,
		FILE
	};
	...
	public static enum SHARED_TYPE {
		PUBLIC,
		PRIVATE
	};

	/** executed sql history type */
	public static enum EXECUTE_SQL_TYPE {
		EDITOR,
		SESSION
	};
	...
	/** sql을 각 언어로 컨버팅 해줍니다 */
	public static enum SQL_TO_APPLICATION {
		Java_StringBuffer,
		PHP
	};
	...
	public static boolean isPK(String key) {
		if (DB_KEY.PRI.toString().equalsIgnoreCase(key))
			return true;
		if (DB_KEY.PK.toString().equalsIgnoreCase(key))
			return true;
	...
	public static boolean isFK(String key) {
		if (DB_KEY.FK.toString().equalsIgnoreCase(key))
			return true;
		return false;
	}

	public static boolean isMUL(String key) {
		if (DB_KEY.MUL.toString().equalsIgnoreCase(key))
			return true;
		return false;
	}
