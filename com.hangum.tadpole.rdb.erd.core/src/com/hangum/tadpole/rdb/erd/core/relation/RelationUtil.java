	...
	public static void calRelation(UserDBDAO userDB, Map<String, Table> mapDBTables, DB db, List<ReferencedTableDAO> referenceTableList) throws Exception {
		...
		for (ReferencedTableDAO refTabDAO : referenceTableList) {
			...
			if (soTabMod != null && tarTabMod != null) {
				...
				if (refTabDAO.getTable_name().equals(refTabDAO.getReferenced_table_name()))
					continue;
				...
				/*
				 * 저장시 아래와 같은 오류가 발생하여 추가한 코드 여유를 가지고 디버깅을 해봐야 하는코드
				 * 
				 * org.eclipse.emf.ecore.resource.Resource$IOWrappedException:
				 * The object
				 * 'com.hangum.tadpole.model.impl.RelationImpl@5e44efa0
				 * (source_kind: ONLY_ONE, target_kind: ONE_MANY, column_name:
				 * country_id, referenced_column_name: country_id, bendpoint:
				 * [], constraint_name: null)' is not contained in a resource.
				 */
				relation.setDb(db);
				...
				// logger.debug("\t [source ]" + col.getField() + " [key]" +
				// col.getKey());
				// logger.debug("\t [target ]" + colR.getField() + " [key]" +
				// colR.getKey());
