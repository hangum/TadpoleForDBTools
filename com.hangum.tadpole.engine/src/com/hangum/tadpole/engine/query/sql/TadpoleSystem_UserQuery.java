...
public class TadpoleSystem_UserQuery {
	...
	public static UserDAO newLDAPUser(String userName, String email, String external_id, String useOPT) throws TadpoleSQLManagerException, SQLException, Exception {
		return newUser(PublicTadpoleDefine.LOGIN_INPUT_TYPE.LDAP.name(), email, "LDAP", "YES", Utils.getUniqueDigit(12), PublicTadpoleDefine.USER_ROLE_TYPE.USER.name(),
				userName, "KO", "Asia/Seoul", GetAdminPreference.getNewUserPermit(), useOPT, "", "*", external_id, new Timestamp(System.currentTimeMillis()));
	}
	 ...
	 * 
	 */
	private static void licenseLimitCheck() throws Exception {
		if(LicenseValidator.isEnterprise()) {
			LicenseDAO licenseDAO = LicenseValidator.getLicense();
			int intPersonLimit = licenseDAO.getPersonLimit();
			
			int intUserCnt = TadpoleSystem_UserQuery.getAllUserCount() + 1;
			if(intUserCnt > intPersonLimit) {
				throw new Exception(String.format(CommonMessages.get().TadpoleHubLicenseOver, intPersonLimit));
			}
		} else {
			int intUserCnt = TadpoleSystem_UserQuery.getAllUserCount() + 1;
			if(intUserCnt > 5) {
				throw new Exception(String.format(CommonMessages.get().TadpoleHubLicenseOver, 5));
			}
		}
	}
