...
public class HivePreparedStatement implements PreparedStatement {
   ...
   *
   */
	public HivePreparedStatement(TCLIService.Iface client, TSessionHandle sessHandle, String sql) {
		this.client = client;
	...
	private int getCharIndexFromSqlByParamLocation(final String sql, final char cchar, final int paramLoc) {
		...
		for (int i = 0; i < sql.length(); i++) {
			...
			{
				signalCount++;
			} else if (c == cchar && signalCount % 2 == 0) {// check if the ? is
															// really the
															// parameter
				num++;
				if (num == paramLoc) {
