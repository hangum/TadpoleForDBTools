 ...
 *
 */
public class MainEditorFileuploadDialog extends Dialog {
	private static final Logger logger = Logger.getLogger(MainEditorFileuploadDialog.class);
	 ...
	 * @param parentShell
	 */
	public MainEditorFileuploadDialog(Shell parentShell, String strTitle) {
		super(parentShell);
...
//				if(!MessageDialog.openConfirm(null, CommonMessages.get().Confirm, "Do you want file upload?")) return;
	...
	private boolean insert() throws Exception {
		...
		try {
			...
			String charsetName = bom == null ? "UTF-8" : bom.getCharsetName(); //$NON-NLS-1$

			strFileName = userUploadFile.getName();
