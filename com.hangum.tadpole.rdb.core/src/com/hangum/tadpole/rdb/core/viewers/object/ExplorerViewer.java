...
public class ExplorerViewer extends ViewPart {
	...
	public void initObjectHead(Object selectElement) {
		...
		// elaticsearch
		if(null != elasticComposite) elasticComposite.dispose();
	...
	private void initSchema() throws Exception {
		...
		if(userDB.getDBGroup() == DBGroupDefine.POSTGRE_GROUP) {
			...
			if(userDB.getDBDefine() == DBDefine.ELASTICSEARCH_DEFAULT) {
				comboSchema.add("Elasticsearch_default");
				comboSchema.setText("Elasticsearch_default");
			} else {
	...
	private void initObjectDetail(final UserDBDAO userDB) {
		...
		// altibase, cubrid
			...
			getViewSite().setSelectionProvider(new SelectionProviderMediator(arrayStructuredViewer, tableComposite.getTableListViewer()));
		} else if (DBDefine.ELASTICSEARCH_DEFAULT == userDB.getDBDefine()) {
			createElastisearchIndex();
			arrayStructuredViewer = new StructuredViewer[] { 
					elasticComposite.getTableviewer()
			};
	...
	private void refershSelectObject(String strSelectItemText, boolean isRefresh, String strObjectName) {
		...
		} else if (strSelectItemText.equalsIgnoreCase(OBJECT_TYPE.ELASTICSEARCH_INDEX.name())) {
			refreshElasicsearch(isRefresh, strObjectName);
		}
	 ...
	 * Elasticsearch 정의
	 */
	private void createElastisearchIndex() {
		elasticComposite = new ElasticsearchIndexComposite(getSite(), tabFolderObject, userDB);
		elasticComposite.initAction();
		tabFolderObject.setSelection(0);
	}
	...
	public void refreshTable(boolean boolRefresh, String strObjectName) {
		...
		} else if(userDB != null && DBDefine.ELASTICSEARCH_DEFAULT == userDB.getDBDefine()) {
			elasticComposite.refreshIndex(userDB, boolRefresh, strObjectName);
		} else {
	...
	/**
	 * elasticsearch 정보를 최신으로 리프레쉬합니다.
	 */
	public void refreshElasicsearch(boolean boolRefresh, String strObjectName) {
		elasticComposite.refreshIndex(userDB, boolRefresh, strObjectName);
	}
