...
import com.hangum.tadpole.elasticsearch.core.editos.main.ElasticsearchEditor;
import com.hangum.tadpole.elasticsearch.core.editos.main.ElasticsearchEditorInput;
...
public class FindEditorAndWriteQueryUtil {
	...
	public static void run(String strObject) {
		...
		if(editor != null) {				
			...
			MessageDialog.openInformation(null, CommonMessages.get().Information, CommonMessages.get().PleaseOpenEditor);//"에디터를 오픈하여 주십시오.");
		}
	}
	...
	public static void run(UserDBDAO userDB, String strObjectName, String strScript, boolean isNewEditor, PublicTadpoleDefine.OBJECT_TYPE initAction) {
		...
		if(userDB != null && DBGroupDefine.MONGODB_GROUP == userDB.getDBGroup()) {
			...
			if(editor == null || isNewEditor) {				
				newElastisearchEditorOpen(userDB, strScript);	
			} else {
				appendElasticsearchEditorOpen(editor, strScript);				
			}	// end reference
		} else {
...
//			}
	 ...
	 * @param lowSQL
	 */
	private static void newElastisearchEditorOpen(UserDBDAO userDB, String lowSQL) { 
		IWorkbenchPage page = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage();		
		try {
			ElasticsearchEditorInput input = new ElasticsearchEditorInput(userDB, lowSQL);
			page.openEditor(input, ElasticsearchEditor.ID, false);
	 ...
	 * @param lowSQL
	 */
	private static void appendElasticsearchEditorOpen(IEditorPart editorPart, String lowSQL) {
		try {
			ElasticsearchEditor editor = (ElasticsearchEditor)editorPart;
			PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().openEditor(editor.getEditorInput(), ElasticsearchEditor.ID, false);
