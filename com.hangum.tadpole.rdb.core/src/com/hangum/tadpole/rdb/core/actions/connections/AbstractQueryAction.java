...
public abstract class AbstractQueryAction implements IViewActionDelegate {
	...
	public IEditorPart open(UserDBDAO userDB) {
		...
		if(userDB.getDBDefine() == DBDefine.MONGODB_DEFAULT) {
			...
			try {
				return PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().openEditor(new ElasticsearchEditorInput(userDB), ElasticsearchEditor.ID);
			} catch (PartInitException e) {
				logger.error("open editor", e); //$NON-NLS-1$
	...
	public void run(UserDBResourceDAO dao) {
		...
		if(reference == null) {
			if(PublicTadpoleDefine.RESOURCE_TYPE.OBJECT.name().equals(dao.getResource_types())) {
				...
				if(dao.getParent().getDBDefine() == DBDefine.ELASTICSEARCH_DEFAULT) {
					runElasticsearchEditor(dao);
				} else {
					try {
	 ...
	 * @param dao
	 */
	private void runObjectEditor(UserDBResourceDAO dao) {
		try {
	...
	/**
	 * run elasticsearch editor
	 * @param reference
	 * @param dao
	 */
	private void runElasticsearchEditor(UserDBResourceDAO dao) {
		try {
			ElasticsearchEditorInput mei = new ElasticsearchEditorInput(dao);
			PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().openEditor(mei, ElasticsearchEditor.ID);
		} catch (Exception e) {
			logger.error("new editor", e); //$NON-NLS-1$
