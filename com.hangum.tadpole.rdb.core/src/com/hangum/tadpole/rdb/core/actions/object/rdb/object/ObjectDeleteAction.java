	...
	public void run(IStructuredSelection selection, UserDBDAO userDB, DB_ACTION actionType) {
		if (actionType == PublicTadpoleDefine.DB_ACTION.TABLES) {
			...
			if (selection.getFirstElement() instanceof InformationSchemaDAO) {
				...
				if (MessageDialog.openConfirm(getWindow().getShell(), Messages.ObjectDeleteAction_14, indexDAO.getTABLE_NAME() + Messages.ObjectDeleteAction_15
						+ indexDAO.getINDEX_NAME() + Messages.ObjectDeleteAction_16)) {
				...
				if (MessageDialog.openConfirm(getWindow().getShell(), Messages.ObjectDeleteAction_14, indexDAO.getNs()
						+ " [ " + indexDAO.getName() + "] " + Messages.ObjectDeleteAction_16)) { //$NON-NLS-1$ //$NON-NLS-2$
					try {
