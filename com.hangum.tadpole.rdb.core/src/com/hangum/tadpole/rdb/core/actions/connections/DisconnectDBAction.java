...
public class DisconnectDBAction implements IViewActionDelegate {
	...
	public void run(IAction action) {
		...
		// Connection tree 삭제
		// final ManagerViewer managerView =
		// (ManagerViewer)PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().findView(ManagerViewer.ID);
		...
		Display.getCurrent().asyncExec(new Runnable() {
			@Override
			public void run() {
				// explorerView.initObjectHead(new
				// ManagerListDTO(userDB.getDisplay_name(),
				// DBDefine.getDBDefine(userDB.getTypes()) ));
			}
		}); // end display
		...
		try {
			...
			ExceptionDetailsErrorDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
					"Error", "Disconnection Exception", errStatus); //$NON-NLS-1$
		}
	}
