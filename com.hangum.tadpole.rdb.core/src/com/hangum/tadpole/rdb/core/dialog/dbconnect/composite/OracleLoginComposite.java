 ...
 * 
 * SID jdbc:oracle:thin:@//hostname:port:sid Service Name
...
public class OracleLoginComposite extends AbstractLoginComposite {
	...
	public boolean makeUserDBDao() {
		...
		if (comboConnType.getText().equals("SID")) {
			...
			dbUrl = String.format("jdbc:oracle:thin:@%s:%s/%s", StringUtils.trimToEmpty(textHost.getText()), StringUtils.trimToEmpty(textPort.getText()),
					StringUtils.trimToEmpty(textDatabase.getText()));
		}
