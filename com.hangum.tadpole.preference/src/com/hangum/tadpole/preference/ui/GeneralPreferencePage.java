...
public class GeneralPreferencePage extends TadpoleDefaulPreferencePage implements IWorkbenchPreferencePage {
	...
	public boolean isValid() {
		...
		if(!NumberUtils.isNumber(txtSessionTime)) {
			...
			return false;
		} else if(!((NumberUtils.toInt(txtSessionTime) >= TadpoleProperties.SESSION_TIMEOUT_MIN) 
				&& (NumberUtils.toInt(txtSessionTime) <= TadpoleProperties.SESSION_TIMEOUT_MAX))) {
			textSessionTime.setFocus();
			setValid(false);
			setErrorMessage(String.format(CommonMessages.get().InvalidRange_GEAndLEWithItem, 
                    Messages.get().SessionTimeout_mins,  TadpoleProperties.SESSION_TIMEOUT_MIN, TadpoleProperties.SESSION_TIMEOUT_MAX));
