...
//		btnExtendSearch.setData(PreferenceDefine.MONGO_DEFAULT_FIND_EXTEND);
	...
	public boolean performOk() {
		...
		try {
			...
			MessageDialog.openError(getShell(), Messages.get().Confirm, Messages.get().MongoDBPreferencePage_10);			 //$NON-NLS-1$
			return false;
		}
		try {
			Integer.parseInt(txtMacCount);
		} catch(Exception e) {
			MessageDialog.openError(getShell(), Messages.get().Confirm, Messages.get().MongoDBPreferencePage_11);			 //$NON-NLS-1$
			return false;
		}
