...
public class NewUserDialog extends Dialog {
	...
	private void initUserGroup() {
		...
		if (comboUserGroup != null)
			comboUserGroup.dispose();
		if (textUserGroup != null)
			textUserGroup.dispose();
		...
		if (btnUser.getSelection() || btnDBA.getSelection()) {
			...
			try {
				...
				for (UserGroupDAO userGroupDAO : listUserGroup) {
					...
					if (groupSeq == userGroupDAO.getSeq())
						groupName = userGroupDAO.getName();
	...
	protected void okPressed() {
		...
		try {
			...
			// SessionManager.newLogin(loginDAO.getSeq(), strEmail, name,
			// loginDAO.getUser_type());
		} catch (Exception e) {
			logger.error(Messages.NewUserDialog_8, e);
